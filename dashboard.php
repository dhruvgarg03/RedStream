...
$qy = "SELECT name, email, PhoneNO,password, bloodgroup, gender, birthdate,weight, state, zipcode, district, area, landmark FROM registered_users WHERE email = '$email'";
$stmt=mysqli_query($conn,$qy);
if (!$stmt) {
...
$donations = 0;
$received = 0;
$qq = "SELECT email FROM donations WHERE email = '$email'";
$cc = mysqli_query($conn,$qq);
if ($cc) {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["change_password"])) {
        ...
        $qt = "SELECT password FROM registered_users WHERE email = '$email'";
        $mm = mysqli_query($conn,$qt);
        $row = mysqli_fetch_array($mm);
        if($row["password"]===$current_password) {
            ...
            $sql = "UPDATE registered_users SET password='$new_password' WHERE email='$email'";
        ...
        $sql = "UPDATE registered_users SET name='$name', PhoneNO='$phone', bloodgroup='$bloodgroup', gender='$gender', birthdate='$birthdate', weight='$weight', state='$state', zipcode='$zipcode', district='$district', area='$area', landmark='$landmark' WHERE email='$email'";
